...
6. **完成仓库版本后再同步全局。** 重写前保留可回退的 Git 基线，不覆盖其他未提交工作；讨论期间修改仓库副本，最终确认后再同步全局 `AGENTS.md` 并检查一致性。只替换 Memory 章节，不连带改动 Python、CLI 规则或 Claude Code 的 `CLAUDE.md`。`HERMES.md` 由 `AGENTS.md` 生成，另带 frontmatter 和一段 Hermes 适配：`AGENTS.md` 定稿后按根 `AGENTS.md` 的说明重新生成，并复核那一段是否仍与新原文衔接；设计理由见 [Hermes spec](../docs/specs/20260926-hermes-agent-instructions-spec.md)。更新本指南及来源索引中发生变化的事实，不追加每次编辑的流水账。
