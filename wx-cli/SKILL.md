...
V2 image key 提取（macOS / Windows 自动；**Linux 暂不支持** → 见下方「Linux / webtop 实际取法」）：
...
#### Linux / webtop 实际取法（绕过未实现的 image key）

Linux 上 `wx extract` 对 V2 图片（头 `07 08 56 32 08 07`）会失败：报 **`需要 image AES key`**（上游 Linux 解码模块没填）。⚠️ **此时进程退出码仍是 0**，错误只打到 stderr——别用退出码判断成功，要看输出文件是否真生成。但**微信在 app 里显示图片时，会把解密后的明文 JPG/PNG 缓存到 `temp/ImageUtils/`**，直接取用即可，无需任何密钥：

| 角色 | 路径（相对 `xwechat_files/<wxid>/`） |
|------|----------------------------------------|
| 密文本体 | `msg/attach/<md5(chat_username)>/<YYYY-MM>/Img/<md5>[_h\|_t].dat` |
| **明文缓存** | `temp/ImageUtils/<md5>[_h].jpg`（同名 md5，后缀换 `.jpg`，`_h`=高清） |

**前置：解密后的 db 在哪、用什么查。** daemon 把每个库解密成 `~/.wx-cli/cache/<hash>.db`，**文件名是哈希、不是逻辑名**；逻辑名→文件名的映射在 `~/.wx-cli/cache/_mtimes.json`（key 形如 `contact/contact.db`、`message/message_resource.db`）。本环境**没有 `sqlite3` CLI**，用 `python3` 自带的 sqlite3 模块查：

```python
import json, sqlite3, hashlib, pathlib, re
cache = pathlib.Path.home() / ".wx-cli/cache"
mt = json.load(open(cache / "_mtimes.json"))
dbpath = lambda logical: mt[logical]["path"]        # 如 dbpath("contact/contact.db")
```

取某会话的图片：

1. **群名 → username**。`wx history`/`attachments` 的模糊匹配只认 contact 的部分字段，群名（如「技术交流群」）常匹配不到。去 `contact.db` 的 **`contact` 表**按 `nick_name` 反查 `username`（群是 `*@chatroom`）：
   ```python
   c = sqlite3.connect(dbpath("contact/contact.db"))
   (username,) = c.execute("SELECT username FROM contact WHERE nick_name=?", ("技术交流群",)).fetchone()
   # → '12345678901@chatroom'
   ```
   拿到 username 后，`wx attachments "<username>" --kind image --since YYYY-MM-DD` 能正常列出 `local_id`（此命令不解密，Linux 也能用）。`--since`/`--until` **含端点**、按本地日全天计。
2. **定位 attach 子目录**：`md5(chat_username)` 就是 `msg/attach/` 下的子目录名：
   `hashlib.md5(username.encode()).hexdigest()` → `e05048ca3927c5caf689f54bf57faed0`。该目录 `<YYYY-MM>/Img/*.dat` 是这个会话**全部**图片的 md5。
3. **（可选）精确到某几条消息**：要「只取今天发的那几条」，**别信文件 mtime**——`temp/`、`.dat` 的 mtime 是微信**渲染时刻**，不是发图时间。用 `message_resource.db` 把 `local_id` 映射到图片 md5：
   ```python
   r = sqlite3.connect(dbpath("message/message_resource.db"))
   (chat_id,) = r.execute("SELECT rowid FROM ChatName2Id WHERE user_name=?", (username,)).fetchone()  # 注意是 rowid
   blob, = r.execute("SELECT packed_info FROM MessageResourceInfo "
                     "WHERE chat_id=? AND message_local_id=? AND message_local_type=3 "   # 3=图片
                     "ORDER BY message_create_time DESC LIMIT 1", (chat_id, local_id)).fetchone()
   md5 = re.search(rb'[0-9a-f]{32}', blob).group().decode()                                # packed_info 是 BLOB
   ```
4. **去明文缓存取图**：用每个 md5 到 `temp/ImageUtils/` 找同名 `.jpg`（优先 `<md5>_h.jpg` 高清），拷出即可查看。用 `file` 确认是真图片（`JPEG`/`PNG`），不是占位/损坏。

⚠️ `temp/ImageUtils` 是**缓存**：只有微信实际渲染/查看过的图才在里面，可能不全，微信也可能清理它。要保证拿到**全部**原图，仍需补上 Linux image key（待实现）。

#### 其它附件类型（文件 / 视频 / 语音）

`wx attachments` 目前**只列图片**（`--kind` 只接受 `image`/`img`，传别的会被 CLI 拒绝）。但其它类型在本地**大多是明文、直接可取**，根本不用经过工具或解密：

| 类型 | 位置（相对 `xwechat_files/<wxid>/`） | 加密 | 取法 |
|------|--------------------------------------|------|------|
| **文档/文件** | `msg/file/<YYYY-MM>/<原始文件名>` | ❌ 明文 | 直接 `cp`；**保留原文件名+原格式**，pdf/docx/xlsx/pptx/zip/py… 都能直接打开 |
| **视频** | `msg/video/<YYYY-MM>/<md5>.mp4`（配 `<md5>_thumb.jpg`） | ❌ 明文真 MP4 | 直接 `cp`，可直接播放 |
| **图片** | `msg/attach/<md5(chat)>/<YYYY-MM>/Img/<md5>.dat` | ✅ V2/legacy 加密 | 见上一节（走 `temp/ImageUtils` 明文缓存） |
| **合并转发记录里的图** | `msg/attach/<…>/Rec/<id>/Img/*.dat` | ✅ 同图片 | 同上一节 |
| **语音（PTT）** | 找不到独立文件（无 `*.silk`/`*.amr`） | — | 本地无独立文件，疑在 `media_0.db` 内，**未验证** |

说明：

- 文档和视频既是明文、又带原文件名/真格式，**直接按 `msg/file/<月>/` 或 `msg/video/<月>/` 浏览即可**，比图片省事得多。`msg/file/` 里有用户互发的 `.mp3` 录音文件——那是“文件”，跟微信的 PTT 语音消息是两码事。
- 要把某条消息精确对应到文件，思路同图片（查 `message_resource.db`）：`message_local_type` 中 文件=`49`、视频=`43`、语音=`34`、图片=`3`（高 32 位是 flag，取低 32 位）。但文档/视频是明文，多数情况按文件名直接找更快，不必回链。
...
**先看 `meta.status`，再相信结果（重要）。** `history` / `search` / `sessions` / `new-messages` / `stats` / `attachments` 的结果在当作事实上报前，必须先读 `meta.status`：

- `possibly_stale_unknown_shards`：磁盘上有 daemon 没密钥的新分片，**当前结果可能漏消息**。必须先 `wx init --force` 刷新密钥再查；在此之前 `count: 0` 绝不能当成"确实没有"。
- `possibly_stale`：`session.db` 的最新时间明显领先于查到的最新消息，结果可能不全，上报时要说明"可能漏消息"。
- `windowed`：传了 `--since` / `--until` / `offset`，这是局部窗口视图，不要当成"全量最新状态"。
- `ok`：可放心使用。
...
**什么时候需要重跑 `wx init --force`？** 不是每次取消息，也不是微信一重启就要。密钥是 `all_keys.json` 的快照，同账号重启微信后依旧有效，daemon 会用已有密钥自动读到新消息（新消息进的是已有分片，按 mtime 自动重解密）。真正需要重跑的只有两种：① 查询的 `meta.status = possibly_stale_unknown_shards`——磁盘上出现了 daemon 没有密钥的新 `message_N.db` 分片；② 微信退出登录后重新登录 / 重装 / 换设备（密钥本身变了）。重跑时微信必须正在运行（init 要扫其内存）：`sudo wx init --force`。
