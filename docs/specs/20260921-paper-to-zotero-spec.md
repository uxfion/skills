# paper-to-zotero — 设计 spec v9(已实现;真实运行与新 agent 演练通过;整理全库第一批待用户确认)

**状态(2026-09-21)**:v3 经三份独立审查(需求忠实度、技术正确性、agent 可用性)+ 对照 zotero 10.0.3 源码自审;v4 起按用户原则复审并逐条决定(§7);v8 跑完全部探针(§12);v9 是通读整理后的定稿——去掉了历次改通道留下的过时引用,合并了重复段落。**2026-09-22 起脚本、SKILL.md、references 已实现**(§10),剩 §8 第 2–5 步的验证。交接见 §10,修订历史见 §13。
